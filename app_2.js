// Enhanced Persian Poetry Garden - Advanced Interactive JavaScript
class AdvancedPoetryGarden {
    ...
    bindEvents() {
        ...
        poetCards.forEach(card => {
            ...
            card.addEventListener('click', (e) => {
                ...
                setTimeout(() => this.showPoetry(poetId), 300);
            });
        });
        ...
        if (backButton) {
            this.addTouchEffects(backButton);
            backButton.addEventListener('click', (e) => {
                this.createRipple(e.clientX, e.clientY);
                setTimeout(() => this.showGarden(), 200);
            });
        }
    ...
    addPoemSpecialEffects(poemElement) {
        ...
        persianText.addEventListener('mouseenter', () => {
            ...
            const text = persianText.textContent;
            const letters = text.split('');
            persianText.innerHTML = letters.map((letter, index) => 
                `<span style="display: inline-block; animation: floatLetter 2s infinite ease-in-out; animation-delay: ${index * 0.05}s;">${letter}</span>`
            ).join('');
        });

        persianText.addEventListener('mouseleave', () => {
            persianText.style.textShadow = '';
            persianText.innerHTML = persianText.textContent;
        });
    }
    ...
    setupAccessibility() {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Enter' || e.key === ' ') {
                ...
                if (focused.classList.contains('poet-card')) {
                    e.preventDefault();
                    focused.click();
                }
            }
        ...
        const interactiveElements = document.querySelectorAll('.poet-card, .back-button');
